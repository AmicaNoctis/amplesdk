var nClassNumber	= 0,
	...
	aValidKeys	= [
		"extend", "(inherit)",
		"statics", "(stat)", "(consts)", "(constants)",
		"construct", "(ctor)",
		"vars", "(variables)", "(props)", "(properties)", "(atts)", "(attributes)",
		"events", "(signals)",
		"methods", "(meths)", "(funcs)", "(functions)", "(ops)", "(operations)",
	];
...
function fCreateConstructor (fConstruct, sExtends, oVariables, aEvents, sFullClassName, sClsKey) {
	var cClass	= function () {
		...
		for (sSourceName in oVariables) {
			if (oVariables.hasOwnProperty(sSourceName)) {
				...
				if (bPrivate || bProtected) {
					if (typeof oVariables[sSourceName] === "object"
						&& oVariables[sSourceName] !== null
						|| oVariables[sSourceName] instanceof Function
						) {
						sErr	= (bProtected ? "Protected" : "Private") + " variable "
							+ sDestinationName + " must be an atomic value";
						throw new Error(sErr);
					}
		...
		this.toString   = (function (fMethod) {
			return function () {
				var sText   = fMethod ? fMethod.call(this) : sFullClassName;
				sText = sText.match(/^\[.+\]$/) ? sFullClassName : sText;
				return "[object " + sText + "]";
			}
		})(this.toString);
	...
	for (sSourceName in oMethods) {
		if (oMethods.hasOwnProperty(sSourceName)) {
			...
			if (!(oMethods[sSourceName] instanceof Function)) {
				sErr	= "Method " + sFullClassName + "." + sDestinationName
					+ " must be a function";
				throw new Error(sErr);
			}
 ...
 * @param {String} [oConfiguration.extend] Full name of parent class, default "Object".
...
oAmple.defineClass	= function (sFullName, oConfiguration) {
	...
	oConfiguration.construct	= oConfiguration.construct || oConfiguration.ctor || undefined;
	oConfiguration.events	= oConfiguration.events || oConfiguration.signals || undefined;
	oConfiguration.extend	= oConfiguration.extend
		|| oConfiguration.inherit
		|| oConfiguration["extends"]
		|| undefined;
	oConfiguration.vars	= oConfiguration.vars
		|| oConfiguration.variables
		|| oConfiguration.props
		|| oConfiguration.properties
		|| oConfiguration.atts
		|| oConfiguration.attributes
		|| undefined;
	oConfiguration.methods	= oConfiguration.methods
		|| oConfiguration.meths
		|| oConfiguration.funcs
		|| oConfiguration.functions
		|| oConfiguration.ops
		|| oConfiguration.operations
		|| undefined;
	oConfiguration.statics	= oConfiguration.statics
		|| oConfiguration.stat
		|| oConfiguration.consts
		|| oConfiguration.constants
		|| undefined;
	aNamespace	= fResolve(sFullName, true);
	sExtends	= typeof oConfiguration.extend === "string"
		? oConfiguration.extend
		: "Object";
	...
	for (sKey in oConfiguration) {
		if (oConfiguration.hasOwnProperty(sKey) && oConfiguration[sKey] !== undefined) {
			...
			switch (sKey) {
				...
				case "ops":
				case "operations":
					break; // ignore alternate names, they have been merged earlier
				default:
